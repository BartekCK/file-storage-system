...
and clear image from Exif [_(scroll bellow to know something more)_](#clear-image-exif).
...
Resized, without Exif file is saved in **for-share** folder after finish `file-processor` task, which 
...
during processing big image? For this case app use durable queue. You should know that `file-service` send
...
again _(if it was not previously confirmed)_ and all actions will be make to the end. 
